...
### Fixed
- Chat bubbles no longer dump raw upstream model-provider error JSON at the user. `readNdjsonStream` now routes `event:"error"` payloads through a new `friendlyAgentError(detail)` helper that maps known transient failure modes (NVIDIA NIM `DEGRADED function`, rate limits / 429, connection resets, generic timeouts) to human-readable messages while preserving the backend's own already-friendly `Pipeline exceeded Ns total cap...` text verbatim. Raw detail is logged to `console.warn` for developer debugging. No backend or agent change.
